...
namespace ON\request;
// Strongly based in AgaviWebRequest
...
class Request
{
  ...
  public function getUrlAuthority($forcePort = false)
  {
    ...
    return $this->getUrlHost() . ($forcePort || $this->isPortNecessary($scheme, $port) ? ':' . $port : '');
  }
  ...
  public function __construct(array $parameters = array())
  {
    ...
    $this->setMethod($this->getServerValue('REQUEST_METHOD', 'GET'));

    $this->protocol = $this->getServerValue('SERVER_PROTOCOL');
    ...
    $HTTPS = (bool)preg_match('/^(on|https|1)$/i', isset($_SERVER['HTTPS'])? 1 : 0);

    $this->urlScheme = 'http' . ($HTTPS ? 's' : '');

    $this->urlPort = (int) $this->getServerValue('SERVER_PORT', 80);

    $SERVER_NAME = $this->getServerValue('SERVER_NAME');
    $port = $this->getUrlPort();
    ...
    $_SERVER['SERVER_SOFTWARE'] = $this->getServerValue('SERVER_SOFTWARE');
    ...
    } elseif($this->getMethod() == 'POST' && (!isset($_SERVER['CONTENT_TYPE']) || (isset($_SERVER['CONTENT_TYPE']) && !preg_match('#^(application/x-www-form-urlencoded|multipart/form-data)(;[^;]+)*?$#', $_SERVER['CONTENT_TYPE'])))) {
    ...
    } elseif($this->getMethod() == 'POST' && isset($_SERVER['CONTENT_TYPE']) && preg_match('#^multipart/form-data(;[^;]+)*?$#', $_SERVER['CONTENT_TYPE'])) {
