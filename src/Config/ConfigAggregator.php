...
EOT;
    ...
    public function updateCache(bool $force = false) {
        $this->cacheConfig($this->config, $this->cachedConfigFile, $force);
    }

    public function injectNewProvider($provider) {
     ...
     * @throws InvalidConfigProviderException
     */
    public function mergeConfig(array &$mergedConfig, mixed $config, callable $provider): void
    {
        if (! is_array($config)) {
            $type = $this->detectVariableType($provider);
    ...
    public function injectConfigFromProvider($provider, array &$mergedConfig) {
        $config = $this->getConfigFromProvider($provider);
        $provider = $this->resolveProvider($provider);
     ...
     */
    protected function loadConfigFromProviders(iterable $providers): array
    {
        $mergedConfig = [];
        foreach ($providers as $provider) {
            $this->injectConfigFromProvider($provider, $mergedConfig);
        }
        return $mergedConfig;
    }
     ...
     */
    public function loadConfigFromCache(null|string $cachedConfigFile): bool
    {
     ...
     */
    public function cacheConfig(array $config, null|string $cachedConfigFile, bool $force = false): void
    {

        if (null === $cachedConfigFile) {
            if ($force) {
                throw new Exception("There is no filename defined to cache the config file");
            }
            return;
        }
        
        if (empty($config[static::ENABLE_CACHE]) && !$force) {
            return;
        }
