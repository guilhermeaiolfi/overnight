 ...
 */
class GlobMTimeResource extends GlobResource implements \IteratorAggregate, SelfCheckingResourceInterface
{
    ...
    public function getIterator(): \Traversable
    {
        ...
        if ('' === $this->pattern && is_file($this->prefix)) {
            ...
            if (!str_contains($this->pattern, '/**/') && ($this->globBrace || !str_contains($this->pattern, '{'))) {
                //var_dump(array_fill_keys(glob($this->pattern, \GLOB_NOSORT | $this->globBrace), $this->globBrace), null);exit;
                //echo "aa" . $this->prefix . "||||" . $this->pattern;exit;
...
//d($this->getIterator()->current());exit;
        foreach ($this->getIterator() as $path => $info) {
            //var_dump($info);
            $mtime = @filemtime($path);
            hash_update($hash, $path. $mtime . "\n");
        }
